...
import BrandResource from '@/api/brand/BrandResource';
import BrandForm from '@/components/Brand/BrandForm';

const resource = new BrandResource();
...
const getters = {
  ...
  'form/title': (state, getters) => (getters['form/value/id'] ? 'Edit Brand' : 'Add Brand'),
...
const actions = {
  ...
  create: async ({ commit, getters, dispatch }) => {
    ...
    if (status === 201) {
      await dispatch('appsnackbar/set', { show: true, text: 'Brand has been successfully added.' }, { root: true });
  ...
  update: async ({ commit, getters, dispatch }) => {
    ...
    commit('FORM/SET', { status, errors: (data.errors || {}), loading: false });
    if (status === 200) {
      await dispatch('appsnackbar/set', { show: true, text: 'Brand has been successfully updated.' }, { root: true });
      await dispatch('appdialog/reset', {}, { root: true });
      await dispatch('list/get');
    }
  },
